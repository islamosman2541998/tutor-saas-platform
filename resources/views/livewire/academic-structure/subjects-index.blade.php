<div>
    <div class="mb-6 flex items-center justify-between">
        <div>
            ...
            <p class="text-sm text-slate-500">المواد التي تُدرّسها، وتُستخدم عند إنشاء المجموعات</p>
        </div>
    ...
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        @forelse ($subjects as $subject)
            <div wire:key="subject-{{ $subject->id }}" class="rounded-2xl border border-slate-200 bg-white p-5">
                <div class="mb-3 flex items-center gap-3">
                    ...
                    @else
                        <span class="flex h-12 w-12 items-center justify-center rounded-lg bg-slate-100 text-slate-400">؟</span>
                    @endif
                ...
                @if ($subject->description)
                    <p class="mb-4 text-sm text-slate-500">{{ $subject->description }}</p>
                @endif

                <div class="flex gap-2">
                    <button wire:click="edit({{ $subject->id }})" class="rounded-lg border border-slate-300 px-3 py-1.5 text-xs font-medium text-slate-700 hover:bg-slate-50">
                        تعديل
                    </button>
    ...
    <x-ui.modal :show="$showForm" :title="$editingId ? 'تعديل المادة' : 'مادة جديدة'" on-close="$set('showForm', false)">
        <form wire:submit="save" class="space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    <label class="mb-1.5 block text-sm font-medium text-slate-700">صورة (اختياري)</label>
