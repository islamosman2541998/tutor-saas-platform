<div>
    <div class="mb-6 flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        <div>
            ...
            <p class="text-sm text-slate-500">إدارة حسابات المدرسين واشتراكاتهم</p>
        </div>
            ...
            <tbody class="divide-y divide-slate-100">
                @forelse ($tenants as $tenant)
                    ...
                    <tr wire:key="tenant-{{ $tenant->id }}">
                        <td class="px-4 py-3">
                            <div class="font-medium text-slate-900">{{ $tenant->teacher_name }}</div>
                        ...
                        <td class="px-4 py-3">
                            ...
                            @else
                                <span class="text-xs text-slate-400">بدون اشتراك</span>
                            @endif
                        </td>
                        <td class="px-4 py-3">
                            @php
                        ...
                        <td class="px-4 py-3 text-slate-500">{{ $tenant->created_at->format('Y-m-d') }}</td>
                        <td class="px-4 py-3">
                ...
                @empty
                    <tr>
                        <td colspan="6" class="px-4 py-10 text-center text-slate-400">لا يوجد مدرسون بعد.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
    ...
    <x-ui.modal :show="$managingTenantId !== null" title="إدارة اشتراك المدرس" on-close="closeSubscriptionForm">
        <form wire:submit="saveSubscription" class="space-y-4">
            <div>
                <label class="mb-1.5 block text-sm font-medium text-slate-700">الباقة</label>
                <select wire:model.live="plan_id" class="block w-full rounded-lg border border-slate-300 px-3.5 py-2.5 text-sm focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-200">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label class="mb-1.5 block text-sm font-medium text-slate-700">دورة الفوترة</label>
                    <select wire:model.live="billing_cycle" class="block w-full rounded-lg border border-slate-300 px-3.5 py-2.5 text-sm focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-200">
                ...
                <div>
                    <label class="mb-1.5 block text-sm font-medium text-slate-700">حالة الاشتراك</label>
                    <select wire:model="subscription_status" class="block w-full rounded-lg border border-slate-300 px-3.5 py-2.5 text-sm focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-200">
