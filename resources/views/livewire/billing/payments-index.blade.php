<div>
    <div class="mb-6 flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        <div>
            ...
            <p class="text-sm text-slate-500">سجل الدفعات المستلمة من الطلاب وإيصالاتها</p>
        </div>
    ...
    <div class="mb-6 rounded-2xl border border-slate-200 bg-white p-4">
        <p class="text-xs font-medium text-slate-400">إجمالي الدفعات المكتملة (حسب الفلاتر الحالية)</p>
    ...
    <div class="mb-4 flex flex-wrap gap-2">
        ...
        <select wire:model.live="methodFilter" class="rounded-lg border border-slate-300 px-3.5 py-2 text-sm focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-200">
        ...
        <input type="date" wire:model.live="fromDate" class="rounded-lg border border-slate-300 px-3.5 py-2 text-sm focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-200">
        <input type="date" wire:model.live="toDate" class="rounded-lg border border-slate-300 px-3.5 py-2 text-sm focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-200">
    </div>
            ...
            <tbody class="divide-y divide-slate-100">
                @forelse ($payments as $payment)
                    <tr wire:key="payment-{{ $payment->id }}">
                        <td class="px-4 py-3 font-medium text-slate-900">{{ $payment->receipt_number }}</td>
                        <td class="px-4 py-3">
                            <div class="font-medium text-slate-900">{{ $payment->student->name }}</div>
                            <div class="text-xs text-slate-500">{{ $payment->student->student_code }}</div>
                        </td>
                        <td class="px-4 py-3 text-slate-600">
                            ...
                            @else
                                <span class="text-xs text-slate-400">رصيد مقدَّم</span>
                            @endif
                        </td>
                        <td class="px-4 py-3 text-slate-600">{{ number_format((float) $payment->amount, 2) }}</td>
                        <td class="px-4 py-3 text-slate-600">{{ $payment->methodLabel() }}</td>
                        <td class="px-4 py-3 text-slate-600">{{ $payment->paid_at->format('Y-m-d H:i') }}</td>
                        <td class="px-4 py-3">
                            @php
                        ...
                        <td class="px-4 py-3">
                            <div class="flex flex-wrap gap-2">
                                <a href="{{ route('tenant.payments.receipt', ['tenant' => $currentTenant->slug, 'payment' => $payment->id]) }}" target="_blank" class="rounded-lg border border-slate-300 px-2.5 py-1.5 text-xs font-medium text-slate-700 hover:bg-slate-50">
                                    الإيصال
                                </a>
                                @can('payments.cancel')
    ...
    <x-ui.modal :show="$showRecordForm" title="تسجيل دفعة" on-close="$set('showRecordForm', false)">
        <form wire:submit="recordPayment" class="max-h-[75vh] space-y-4 overflow-y-auto pe-1">
            ...
            @if ($selectedStudentId)
                <div>
                    <label class="mb-1.5 block text-sm font-medium text-slate-700">الاستحقاقات المستحقة</label>

                    @if ($outstandingDues->isEmpty())
                        <p class="rounded-lg border border-slate-200 bg-slate-50 p-3 text-sm text-slate-500">
                        ...
                        <div class="space-y-2">
                            @foreach ($outstandingDues as $due)
                                <div class="flex items-center gap-3 rounded-lg border border-slate-200 p-3">
                                    <input type="checkbox" wire:model="selectedDues.{{ $due->id }}" class="rounded border-slate-300 text-indigo-600">
                                    <div class="flex-1 text-sm">
                                        <div class="font-medium text-slate-900">{{ \App\Models\MonthlyDue::monthLabel($due->billing_month) }} {{ $due->billing_year }}</div>
                                        <div class="text-xs text-slate-500">المتبقي: {{ number_format((float) $due->remaining_amount, 2) }}</div>
                                    </div>
                                    <input
                                        type="number" step="0.01"
                                        wire:model="dueAmounts.{{ $due->id }}"
                                        class="w-28 rounded-lg border border-slate-300 px-2.5 py-1.5 text-sm focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-200"
                                    >
                                </div>
                            @endforeach
                        </div>
            ...
            <div>
                <label class="mb-1.5 block text-sm font-medium text-slate-700">مرفق إثبات الدفع (اختياري)</label>
    ...
    <x-ui.modal :show="$showCancelForm" title="إلغاء الدفعة" on-close="$set('showCancelForm', false)">
        <form wire:submit="saveCancel" class="space-y-4">
            <div>
                ...
                <textarea wire:model="cancelReason" rows="3" class="block w-full rounded-lg border border-slate-300 px-3.5 py-2.5 text-sm focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-200"></textarea>
