<div>
    ...
    <div class="mb-6 flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        <div>
            ...
            <p class="text-sm text-slate-500 dark:text-slate-400">{{ $session->group->name }} — {{ $session->scheduled_date->format('Y-m-d') }}</p>
        </div>
    ...
    <div class="mb-4 flex flex-wrap gap-2">
        ...
        <button wire:click="clearAllVisible" class="rounded-lg border border-slate-300 dark:border-slate-600 px-3 py-1.5 text-xs font-medium text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700/50">
            إلغاء التحديد
        </button>
    </div>
                    ...
                    <tr wire:key="att-{{ $studentId }}">
                        ...
                        <td class="px-4 py-3">
                            ...
                            @elseif ($methods[$studentId] ?? null)
                                <span class="text-xs text-slate-400 dark:text-slate-500">يدوي</span>
                            @else
                        ...
                        <td class="px-4 py-3">
                            <select
                                ...
                                class="rounded-lg border border-slate-300 dark:border-slate-600 px-3 py-1.5 text-sm focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-200"
                            >
