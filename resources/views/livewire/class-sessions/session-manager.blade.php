<div @if ($session->status === 'open') wire:poll.5s @endif>
    ...
    <div class="mb-6 flex items-center justify-between">
        <div>
            ...
            <p class="text-sm text-slate-500">{{ substr($session->expected_start_time, 0, 5) }} - {{ substr($session->expected_end_time, 0, 5) }}</p>
        </div>
        @php
    ...
    @if ($session->status === 'scheduled')
        <div class="rounded-2xl border border-slate-200 bg-white p-8 text-center">
            <p class="mb-4 text-slate-500">الحصة لم تبدأ بعد.</p>
    ...
    @elseif ($session->status === 'open')
        <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
            <div class="rounded-2xl border border-slate-200 bg-white p-6 text-center">
                @if ($attendanceUrl)
                    <div class="mx-auto mb-4 inline-block rounded-xl bg-white p-4 shadow-sm">
                    ...
                    <div class="flex flex-wrap justify-center gap-2">
                        <button wire:click="regenerateQr" class="rounded-lg border border-slate-300 px-3 py-1.5 text-xs font-medium text-slate-700 hover:bg-slate-50">
                            توليد رمز جديد
                        </button>
                        <button wire:click="extendQr" class="rounded-lg border border-slate-300 px-3 py-1.5 text-xs font-medium text-slate-700 hover:bg-slate-50">
                            تمديد 10 دقائق
                        </button>
                ...
                <div class="mt-6 flex flex-wrap justify-center gap-2 border-t border-slate-100 pt-4">
                    <a href="{{ route('tenant.sessions.attendance', ['tenant' => $currentTenant->slug, 'session' => $session->id]) }}" class="rounded-lg border border-slate-300 px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50">
                        تسجيل حضور يدوي
                    </a>
            ...
            <div class="rounded-2xl border border-slate-200 bg-white p-6">
                ...
                <ul class="space-y-2">
                    @forelse ($records as $record)
                        <li wire:key="record-{{ $record->id }}" class="flex items-center justify-between rounded-lg bg-slate-50 px-3 py-2 text-sm">
                            <span>{{ $record->student->name }}</span>
                            <span class="text-xs text-slate-400">{{ $record->checked_in_at?->format('H:i') }}</span>
                        </li>
                    @empty
                        <li class="text-center text-sm text-slate-400">لا يوجد حضور مسجَّل حتى الآن.</li>
                    @endforelse
                </ul>
            </div>
        </div>
