<div>
    <div class="mb-6 flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        <div>
            ...
            <p class="text-sm text-slate-500">طلبات الانضمام اللي وصلت من الموقع التعريفي</p>
        </div>
            ...
            <tbody class="divide-y divide-slate-100">
                @forelse ($requests as $request)
                    <tr wire:key="registration-request-{{ $request->id }}">
                        <td class="px-4 py-3 font-medium text-slate-900">{{ $request->student_name }}</td>
                        <td class="px-4 py-3 text-slate-600">{{ $request->phone }}</td>
                        <td class="px-4 py-3 text-slate-600">{{ $request->guardian_phone ?: '—' }}</td>
                        <td class="px-4 py-3 text-slate-600">{{ $request->group->name }}</td>
                        <td class="px-4 py-3">
                            @php
                        ...
                        <td class="px-4 py-3 text-slate-500">{{ $request->created_at->format('Y-m-d') }}</td>
                        <td class="px-4 py-3">
                ...
                @empty
                    <tr>
                        <td colspan="7" class="px-4 py-10 text-center text-slate-400">لا يوجد طلبات تسجيل بعد.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
