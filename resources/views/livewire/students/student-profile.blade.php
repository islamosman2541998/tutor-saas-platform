<div>
    ...
    <div class="mb-6 flex items-center gap-4">
        ...
        <div>
            ...
            <p class="text-sm text-slate-500">{{ $student->student_code }} — {{ $student->statusLabel() }}</p>
        </div>
    </div>
    ...
    <div class="mb-6 grid grid-cols-1 gap-4 sm:grid-cols-2">
        <div class="rounded-2xl border border-slate-200 bg-white p-5">
            ...
            <dl class="space-y-2 text-sm">
                <div class="flex justify-between"><dt class="text-slate-500">الهاتف</dt><dd>{{ $student->phone ?: '—' }}</dd></div>
                <div class="flex justify-between"><dt class="text-slate-500">البريد الإلكتروني</dt><dd>{{ $student->email ?: '—' }}</dd></div>
                <div class="flex justify-between"><dt class="text-slate-500">ولي الأمر</dt><dd>{{ $student->guardian_name ?: '—' }}</dd></div>
                <div class="flex justify-between"><dt class="text-slate-500">هاتف ولي الأمر</dt><dd>{{ $student->guardian_phone ?: '—' }}</dd></div>
            </dl>
        ...
        <div class="rounded-2xl border border-slate-200 bg-white p-5">
            ...
            <dl class="space-y-2 text-sm">
                <div class="flex justify-between"><dt class="text-slate-500">المدرسة</dt><dd>{{ $student->school_name ?: '—' }}</dd></div>
                <div class="flex justify-between"><dt class="text-slate-500">المحافظة</dt><dd>{{ $student->governorate ?: '—' }}</dd></div>
                <div class="flex justify-between"><dt class="text-slate-500">تاريخ الانضمام</dt><dd>{{ $student->joined_at->format('Y-m-d') }}</dd></div>
            </dl>
            @if ($student->notes)
                <p class="mt-3 rounded-lg bg-slate-50 p-3 text-xs text-slate-600">{{ $student->notes }}</p>
            @endif
        </div>
    </div>
            ...
            <tbody class="divide-y divide-slate-100">
                @forelse ($enrollments as $enrollment)
                    <tr wire:key="enr-{{ $enrollment->id }}">
                        <td class="px-4 py-3 font-medium text-slate-900">{{ $enrollment->group->name }}</td>
                        <td class="px-4 py-3 text-slate-600">{{ $enrollment->final_monthly_price }}</td>
                        <td class="px-4 py-3">
                            @php
                        ...
                        <td class="px-4 py-3">
                            @if ($enrollment->status === 'active')
                                <button wire:click="openTransferForm({{ $enrollment->id }})" class="rounded-lg border border-slate-300 px-2.5 py-1.5 text-xs font-medium text-slate-700 hover:bg-slate-50">
                                    نقل مجموعة
                                </button>
                            @endif
                        </td>
    ...
    <x-ui.modal :show="$showTransferForm" title="نقل الطالب لمجموعة أخرى" on-close="$set('showTransferForm', false)">
        <form wire:submit="transfer" class="space-y-4">
            <div>
                <label class="mb-1.5 block text-sm font-medium text-slate-700">المجموعة الجديدة</label>
                <select wire:model="to_group_id" class="block w-full rounded-lg border border-slate-300 px-3.5 py-2.5 text-sm focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-200">
