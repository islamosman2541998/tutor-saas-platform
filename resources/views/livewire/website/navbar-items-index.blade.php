<div>
    <div class="mb-6 flex items-center justify-between">
        <div>
            ...
            <p class="text-sm text-slate-500 dark:text-slate-400">الروابط التي تظهر أعلى موقعك التعريفي</p>
        </div>
                    ...
                    <tr wire:key="navbar-item-{{ $item->id }}">
                        <td class="px-4 py-3">
                            <div class="flex gap-1">
                                <button wire:click="moveUp({{ $item->id }})" @if($index === 0) disabled @endif class="rounded border border-slate-200 dark:border-slate-700 px-1.5 py-0.5 text-xs text-slate-500 dark:text-slate-400 disabled:opacity-30">↑</button>
                                <button wire:click="moveDown({{ $item->id }})" @if($index === $items->count() - 1) disabled @endif class="rounded border border-slate-200 dark:border-slate-700 px-1.5 py-0.5 text-xs text-slate-500 dark:text-slate-400 disabled:opacity-30">↓</button>
                            </div>
                        </td>
                        <td class="px-4 py-3 font-medium text-slate-900 dark:text-slate-100">{{ $item->label }}</td>
                        <td class="px-4 py-3 text-slate-600 dark:text-slate-400">{{ $item->typeLabel() }}</td>
                        ...
                        <td class="px-4 py-3">
                            <div class="flex flex-wrap gap-2">
                                <button wire:click="edit({{ $item->id }})" class="rounded-lg border border-slate-300 dark:border-slate-600 px-2.5 py-1.5 text-xs font-medium text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700/50">
                                    تعديل
                                </button>
    ...
    <x-ui.modal :show="$showForm" :title="$editingId ? 'تعديل العنصر' : 'عنصر جديد'" on-close="$set('showForm', false)">
        <form wire:submit="save" class="space-y-4">
            ...
            <div>
                <label class="mb-1.5 block text-sm font-medium text-slate-700 dark:text-slate-300">النوع</label>
                <select wire:model.live="type" class="block w-full rounded-lg border border-slate-300 dark:border-slate-600 px-3.5 py-2.5 text-sm focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-200">
            ...
            @elseif ($type === 'page')
                <div>
                    <label class="mb-1.5 block text-sm font-medium text-slate-700 dark:text-slate-300">الصفحة</label>
                    <select wire:model="target_key" class="block w-full rounded-lg border border-slate-300 dark:border-slate-600 px-3.5 py-2.5 text-sm focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-200">
            ...
            @else
                <div>
                    <label class="mb-1.5 block text-sm font-medium text-slate-700 dark:text-slate-300">رابط النظام</label>
                    <select wire:model="target_key" class="block w-full rounded-lg border border-slate-300 dark:border-slate-600 px-3.5 py-2.5 text-sm focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-200">
            ...
            <label class="flex items-center gap-2 text-sm text-slate-600 dark:text-slate-400">
                <input type="checkbox" wire:model="is_visible" class="rounded border-slate-300 dark:border-slate-600 text-indigo-600">
