<div>
    <div class="mb-6 flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        <div>
            ...
            <p class="text-sm text-slate-500 dark:text-slate-400">محتوى تعليمي يظهر في قسم "نصائح تهمّك" بموقعك التعريفي</p>
        </div>
                    ...
                    <tr wire:key="post-{{ $post->id }}">
                        <td class="px-4 py-3">
                            <div class="flex items-center gap-3">
                                ...
                                @endif
                                <span class="font-medium text-slate-900 dark:text-slate-100">{{ $post->title }}</span>
                            </div>
                        </td>
                        <td class="px-4 py-3">
                            @php
                        ...
                        <td class="px-4 py-3 text-slate-600 dark:text-slate-400">{{ $post->published_at?->format('Y-m-d') ?? '—' }}</td>
                        <td class="px-4 py-3">{{ $post->is_featured ? '⭐' : '—' }}</td>
                        <td class="px-4 py-3 text-slate-600 dark:text-slate-400">{{ $post->views_count }}</td>
                        <td class="px-4 py-3">
                            <div class="flex flex-wrap gap-2">
                                <button wire:click="edit({{ $post->id }})" class="rounded-lg border border-slate-300 dark:border-slate-600 px-2.5 py-1.5 text-xs font-medium text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700/50">
                                    تعديل
                                </button>
    ...
    <x-ui.modal :show="$showForm" :title="$editingId ? 'تعديل المقال' : 'مقال جديد'" on-close="$set('showForm', false)">
        <form wire:submit="save" class="max-h-[75vh] space-y-4 overflow-y-auto pe-1">
            ...
            <div>
                ...
                <textarea wire:model="content" rows="8" class="block w-full rounded-lg border border-slate-300 dark:border-slate-600 px-3.5 py-2.5 text-sm focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-200"></textarea>
