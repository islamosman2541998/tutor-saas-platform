<div>
    <div class="mb-6 flex items-center justify-between">
        <div>
            ...
            <p class="text-sm text-slate-500">تظهر في الشريط العلوي و/أو الفوتر حسب ما تحدده</p>
        </div>
                    ...
                    <tr wire:key="social-link-{{ $link->id }}">
                        <td class="px-4 py-3">
                            <div class="flex gap-1">
                                <button wire:click="moveUp({{ $link->id }})" @if($index === 0) disabled @endif class="rounded border border-slate-200 px-1.5 py-0.5 text-xs text-slate-500 disabled:opacity-30">↑</button>
                                <button wire:click="moveDown({{ $link->id }})" @if($index === $links->count() - 1) disabled @endif class="rounded border border-slate-200 px-1.5 py-0.5 text-xs text-slate-500 disabled:opacity-30">↓</button>
                            </div>
                        </td>
                        <td class="px-4 py-3 font-medium text-slate-900">{{ $link->platformLabel() }}</td>
                        <td class="px-4 py-3 text-slate-500 text-xs max-w-[220px] truncate">{{ $link->url }}</td>
                        <td class="px-4 py-3 text-slate-600">{{ $link->displayLocationLabel() }}</td>
                        <td class="px-4 py-3">
                            @if ($link->is_active)
                        ...
                        <td class="px-4 py-3">
                            <div class="flex flex-wrap gap-2">
                                <button wire:click="edit({{ $link->id }})" class="rounded-lg border border-slate-300 px-2.5 py-1.5 text-xs font-medium text-slate-700 hover:bg-slate-50">
                                    تعديل
                                </button>
    ...
    <x-ui.modal :show="$showForm" :title="$editingId ? 'تعديل الرابط' : 'رابط جديد'" on-close="$set('showForm', false)">
        <form wire:submit="save" class="space-y-4">
            <div>
                <label class="mb-1.5 block text-sm font-medium text-slate-700">المنصة</label>
                <select wire:model="platform" class="block w-full rounded-lg border border-slate-300 px-3.5 py-2.5 text-sm focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-200">
            ...
            <div>
                <label class="mb-1.5 block text-sm font-medium text-slate-700">مكان الظهور</label>
                <select wire:model="display_location" class="block w-full rounded-lg border border-slate-300 px-3.5 py-2.5 text-sm focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-200">
