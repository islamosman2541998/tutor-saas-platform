<div>
    <div class="mb-6 flex items-center justify-between">
        <div>
            ...
            <p class="text-sm text-slate-500">شهادات الطلاب وأولياء الأمور التي تظهر في موقعك التعريفي</p>
        </div>
                    ...
                    <tr wire:key="testimonial-{{ $testimonial->id }}">
                        <td class="px-4 py-3">
                            <div class="flex gap-1">
                                <button wire:click="moveUp({{ $testimonial->id }})" @if($index === 0) disabled @endif class="rounded border border-slate-200 px-1.5 py-0.5 text-xs text-slate-500 disabled:opacity-30">↑</button>
                                <button wire:click="moveDown({{ $testimonial->id }})" @if($index === $testimonials->count() - 1) disabled @endif class="rounded border border-slate-200 px-1.5 py-0.5 text-xs text-slate-500 disabled:opacity-30">↓</button>
                            </div>
                        </td>
                        <td class="px-4 py-3">
                            <div class="flex items-center gap-3">
                                ...
                                @endif
                                <div>
                                    <div class="font-medium text-slate-900">{{ $testimonial->student_name }}</div>
                                    @if ($testimonial->grade_or_group)
                                        <div class="text-xs text-slate-500">{{ $testimonial->grade_or_group }}</div>
                                    @endif
                                </div>
                        ...
                        <td class="max-w-[280px] truncate px-4 py-3 text-slate-600">{{ $testimonial->content }}</td>
                        ...
                        <td class="px-4 py-3">
                            <div class="flex flex-wrap gap-2">
                                <button wire:click="edit({{ $testimonial->id }})" class="rounded-lg border border-slate-300 px-2.5 py-1.5 text-xs font-medium text-slate-700 hover:bg-slate-50">
                                    تعديل
                                </button>
        ...
        <form wire:submit="save" class="max-h-[75vh] space-y-4 overflow-y-auto pe-1">
            ...
            <div>
                ...
                @if ($existingImage && ! $student_image)
                    <img src="{{ \Illuminate\Support\Facades\Storage::disk('public')->url($existingImage) }}" class="mt-2 h-12 w-12 rounded-full border border-slate-200 object-cover">
                @endif
                @error('student_image') <p class="mt-1.5 text-sm text-red-600">{{ $message }}</p> @enderror
            </div>

            <div>
                <label class="mb-1.5 block text-sm font-medium text-slate-700">نص الرأي</label>
                <textarea wire:model="content" rows="4" class="block w-full rounded-lg border border-slate-300 px-3.5 py-2.5 text-sm focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-200"></textarea>
                @error('content') <p class="mt-1.5 text-sm text-red-600">{{ $message }}</p> @enderror
            </div>

            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label class="mb-1.5 block text-sm font-medium text-slate-700">التقييم</label>
                    <select wire:model="rating" class="block w-full rounded-lg border border-slate-300 px-3.5 py-2.5 text-sm focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-200">
