<div>
    <div class="mb-6">
        ...
        <p class="text-sm text-slate-500 dark:text-slate-400">تحكّم في ترتيب وظهور وعناوين أقسام موقعك التعريفي</p>
    </div>
                    ...
                    <tr wire:key="section-{{ $section->id }}">
                        <td class="px-4 py-3">
                            <div class="flex gap-1">
                                <button wire:click="moveUp({{ $section->id }})" @if($index === 0) disabled @endif class="rounded border border-slate-200 dark:border-slate-700 px-1.5 py-0.5 text-xs text-slate-500 dark:text-slate-400 disabled:opacity-30">↑</button>
                                <button wire:click="moveDown({{ $section->id }})" @if($index === $sections->count() - 1) disabled @endif class="rounded border border-slate-200 dark:border-slate-700 px-1.5 py-0.5 text-xs text-slate-500 dark:text-slate-400 disabled:opacity-30">↓</button>
                            </div>
                        </td>
                        <td class="px-4 py-3">
                            <div class="font-medium text-slate-900 dark:text-slate-100">{{ $section->label() }}</div>
                            @unless ($section->isReady())
                                <div class="text-xs text-amber-600">قريبًا — لا يظهر في الموقع بعد</div>
                            @endunless
                        </td>
                        <td class="px-4 py-3 text-slate-600 dark:text-slate-400">{{ $section->title ?: '—' }}</td>
                        <td class="px-4 py-3">
                        ...
                        <td class="px-4 py-3">
                            <button wire:click="edit({{ $section->id }})" class="rounded-lg border border-slate-300 dark:border-slate-600 px-2.5 py-1.5 text-xs font-medium text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700/50">
                                تعديل العنوان
                            </button>
                        </td>
                    </tr>
